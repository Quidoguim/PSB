...
- **Dupla:** Gabriel de Souza Nunes and Guilherme Guimarães Santos (names as filled into the report cover page).
- **Report status/workflow:** written report is being built incrementally as a `.docx` in [Trabalho1/relatorio/](Trabalho1/relatorio/) (skeleton generated via the `docx` skill, edited back and forth by the user in Word). When the user edits the file directly, re-inspect it (unzip + `merge_runs.py` from the `docx` skill, or read the rendered PDF) before writing further sections — don't assume the last-generated version is still current.
